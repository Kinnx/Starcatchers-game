...
class Runner
  constructor: ->
    ...
    @STAR_JUMP_SPEED = 510 / @FPS # px / frame
...
class Star
  ...
  hit: ->
    ...
    # W = star width + pipe width = 43 + 69 = 112, W / 2 = 56
    ...
    if pipes.length > 0
      ...
      if Math.abs(star_mx - pipe_mx) <= 56
  ...
  state_dead: ->
    ...
    jQuery(document).trigger 'star:dead'
...
class Game
  ...
  _init_events: ->
    ...
    jQuery(document).on 'star:dead', =>
      @over()

    jQuery(document).on 'star:hit', =>
      @star.state_dead()
  ...
  fly: ->
    ...
    @_show('get_ready', 'star', 'tap', 'score')
